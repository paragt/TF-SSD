...
#KERNEL_INIT =tf.truncated_normal_initializer(stddev=0.09)
...
# Priorbox
# ------------------------------------------------------------------------
def ssd_priorbox_layer(image_dim, aspect_ratio, step, min_size, max_size):
  # Match official SSD feature map size
  #pdb.set_trace()
  feat_map_size = int(math.ceil(float(image_dim) / step))
  ...
  #  aspect ratio 1 and sqrt(s_k * s_{k+1})
  ...
  for r in aspect_ratio:
    l_s = min_size * math.sqrt(r) # aspect ratio a_r (e.g. 2)
    s_s = min_size / math.sqrt(r) # aspect ratio 1/a_r (e.g. 1/2)
    list_w.append(l_s)
    list_h.append(s_s)
    list_w.append(s_s)
    list_h.append(l_s)
  ...
  # # spread over the feature map
  list_x = np.arange(0, image_dim, step) + step / 2  
  list_y = list_x                                  # assuming square input
  ...
  priorbox = priorbox / image_dim

  return priorbox, num_anchors


def ssd_create_priorbox(image_dim, aspect_ratios, steps, min_sizes, max_sizes):
  list_priorbox = []
  list_num_anchors = []
...
def get_anchors(anchors_stride,
  ...
  min_sizes = []
  max_sizes = []
  for ratio in range(min_size_ratio, max_size_ratio + 1, step):
    min_sizes.append(input_dim * ratio / 100.)
    max_sizes.append(input_dim * (ratio + step) / 100.)
  min_sizes = [input_dim * smallest_ratio[0] / 100.] + min_sizes #  4/100 --> 7/100
  max_sizes = [input_dim * smallest_ratio[1] / 100.] + max_sizes # 10/100 --> 15/100
...
def encode_gt(labels, boxes, anchors_map, batch_size):
  ...
  def compute_gt(l, b):
    ...
    fg_idx = np.where(max_iou >= FG_MATCH_IOU)[0]
    bg_idx = np.where(max_iou < FG_MATCH_IOU)[0]
    gt_mask[fg_idx] = 1
    gt_mask[bg_idx] = -1
...
def class_graph_fn(feat, num_classes, num_anchors, layer):
  ...
  output = tf.layers.conv2d(inputs=feat,
                            ...
                            activation=None,
                            name="class/" + layer)
  output = tf.reshape(output, [tf.shape(output)[0], -1, num_classes],name='feat_classes' + layer)
  return output
...
def bbox_graph_fn(feat, num_anchors, layer):
  ...
  output = tf.layers.conv2d(inputs=feat,
                            ...
                            activation=None,
                            name="bbox/" + layer)
  output = tf.reshape(output,[tf.shape(output)[0], -1, 4], name='feat_bboxes' + layer)
  return output


def ssd_block(outputs, name, data_format, conv_strides, filter_size, num_filters, padding, activation='relu'):
  ...
  # https://stackoverflow.com/questions/42924324/tensorflows-asymmetric-padding-assumptions
    ...
    for i in range(num_conv):
        ...
        outputs = tf.layers.conv2d(
          ...
          data_format=data_format,
          kernel_initializer=KERNEL_INIT,
          activation=tf.nn.relu6 if activation=='relu6' else tf.nn.relu,
          name=name + "_" + str(i + 1))
    return outputs
...
def ssd_block_bn(outputs, name, data_format, conv_strides, filter_size, num_filters, padding, is_train=False,activation='relu'):
  # Result will be slightly different to caffe due to TF's asymmetric padding
  # https://stackoverflow.com/questions/42924324/tensorflows-asymmetric-padding-assumptions
    ...
    for i in range(num_conv):
        ...
        outputs = batch_norm(outputs, training=is_train, data_format=data_format)
        outputs = tf.nn.relu6(outputs) if activation=='relu6' else tf.nn.relu(outputs)
...
def hard_negative_mining(logits_classes, gt_mask):
  ...
  fg_index = tf.squeeze(tf.where(tf.math.equal(gt_mask, 1)))
  ...
  bg_score = tf.multiply(bg_score, 1 - fg_mask) + fg_mask # fg_mask makes all fg score max
  bg_score = tf.multiply(-1.0, bg_score) #for sorting in descending order?
  bg_v, bg_index = tf.math.top_k(bg_score, k=bg_num) #smallest confidence for bg (i.e., highest for fg)
...
def detect_per_class(scores, bboxes, anchors_map, num_classes, confidence_threshold):
  ...
  def nms_bboxes(prob, box, idx, anchors):
    ...
    x1, y1, x2, y2 = tf.unstack(box, 4, axis=1)
    ...
    mask = tf.image.non_max_suppression(
        boxes, prob, RESULTS_PER_IM, NMS_THRESH)
